...
from nuvolos_collect.exception import NoCollectiblesException, AmbiguousCopySourceException
import shutil
from datetime import datetime as dt
import json

def identify_collectables(assignment_name, assignment_folder):
    collectables = sorted(Path('/files/assignments-review/handin/').glob(f'**/{assignment_name}/**/{assignment_folder}'))
    clog.debug(collectables)
    if len(collectables) == 0:
        raise NoCollectiblesException(f"Found no collectable submissions with assignment_name {assignment_name} and assignment folder {assignment_folder}.")
    return collectables
...
def get_latest_assignment(items):
    pattern_date_time = re.compile(r"(.*)/([0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}:[0-9]{2}:[0-9]{2})_(.*)")
...
def gather_latest_submissions(user_grouped_submission_info):
    filtered_submissions = {k: get_latest_assignment(v) for k,v in user_grouped_submission_info.items()}
...
def copy_submissions(target_folder, filtered_info, assignment_name, assignment_folder):
    ...
    copy_info = []
    for k,v in filtered_info.items():
        src_path = sorted(Path('/files/assignments-review/handin/').glob(f'{k}/{assignment_name}/{v}*/{assignment_folder}'))
        if len(src_path) > 1:
            clog.error(f"Ambiguous source for {k}. Please collect manually.")
            raise AmbiguousCopySourceException(f"Ambiguous source for {k}. Please collect manually.")
        clog.debug(src_path)
    ...
    write_tracker_file(target_folder, copy_metadata)
    return 0

def write_tracker_file(target_folder, data):
